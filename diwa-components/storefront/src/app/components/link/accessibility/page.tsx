...
export default function LinkAccessibilityPage() {
  return (
    <div className="max-w-3xl">
      ...
      <Section title="Screen reader behaviour">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-4">
          ...
          provided, the inner element renders as a native <code className="font-mono text-xs bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] px-1.5 py-0.5 rounded">&lt;a&gt;</code> with
          full native link semantics. When <code className="font-mono text-xs bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] px-1.5 py-0.5 rounded">href</code> is
          omitted the component renders as a non-interactive <code className="font-mono text-xs bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] px-1.5 py-0.5 rounded">&lt;span&gt;</code>.
        </p>
        <AriaTable rows={[
      ...
      <Section title="Icon-only links">
        <p className="text-sm text-[var(--diwa-text-secondary)]">
          When using <code className="font-mono text-xs bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] px-1.5 py-0.5 rounded">hide-label</code>,
          always supply the <code className="font-mono text-xs bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] px-1.5 py-0.5 rounded">label</code> prop
          so screen readers can announce a meaningful accessible name.
        </p>
      </Section>
      ...
      <Section title="Best practices">
        <ul className="space-y-4 text-sm text-[var(--diwa-text-secondary)]">
          ...
          <li>
            <strong className="text-[var(--diwa-text-primary)]">Icon-only links</strong>{' '}
            — always provide the{' '}
            ...
            The visible label is hidden but still announced by screen readers as the accessible name.
          </li>
        </ul>
      </Section>
    </div>
