...
export default function InputDateAccessibilityPage() {
  return (
    <div className="max-w-3xl">
      <Section title="Keyboard interaction">
        <KeyboardTable
          rows={[
            { key: 'Tab', action: 'Move focus to the date input.' },
            { key: 'Shift + Tab', action: 'Move focus away from the date input to the previous element.' },
            { key: 'Type digits', action: 'Enter date segments directly (month, day, year) by typing numeric values.' },
            { key: 'Arrow Up / Down', action: 'Increment or decrement the currently focused date segment.' },
            { key: 'Enter', action: 'Open or close the native browser date picker dialog.' },
          ]}
        />
      </Section>
        ...
        <AriaTable
          rows={[
            ...
            },
            {
              ...
              value: 'ID of the message element',
            ...
            },
            {
              ...
              note: 'Associates the visible label with the inner <input type="date"> via Shadow DOM id association.',
            },
          ]}
        />
      </Section>
      ...
      <Section title="Label association">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The label is associated with the inner <Code>{'<input type="date">'}</Code> via Shadow DOM
          id association. Screen readers announce the label and the current date value on focus.
        </p>
      </Section>

      <Section title="Native date picker">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The component delegates entirely to the browser's native date picker, which is fully
          keyboard-navigable. Users can enter a date by typing directly or through the picker dialog.
          The picker dialog is managed by the browser's own accessibility implementation.
        </p>
      </Section>
      ...
      <Section title="Date format guidance">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          Use the <Code>description</Code> prop to communicate the expected date format when needed,
          particularly for Safari where the field falls back to a plain text input.
        </p>
      </Section>
        ...
        <div className="space-y-4">
          {[
            ...
            },
            {
              ...
              status: 'Pass',
              detail:
                'All functionality is operable via keyboard. Date segments can be entered by typing or adjusted with arrow keys.',
            },
            {
            ...
            },
            {
              ...
              status: 'Pass',
              detail:
                'The inner <input type="date"> exposes correct name, role, and value to assistive technologies via delegated focus and ARIA labelling.',
            },
      ...
      <Section title="Best practices">
        <ul className="space-y-4 text-sm text-[var(--diwa-text-secondary)]">
          ...
          <li>
            <strong className="text-[var(--diwa-text-primary)]">Safari fallback</strong>{' '}
            — Safari may render the native date picker differently or fall back to a plain text field. Always provide a{' '}
            <code className="px-1 py-0.5 rounded bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] text-xs font-mono">placeholder</code>{' '}
            or description that communicates the expected format (e.g.{' '}
            <code className="px-1 py-0.5 rounded bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] text-xs font-mono">YYYY-MM-DD</code>) so users know what to enter.
          </li>
          <li>
