...
export default function InputTimeAccessibilityPage() {
  return (
    <div className="max-w-3xl">
      <Section title="Keyboard interaction">
        <KeyboardTable
          rows={[
            { key: 'Tab', action: 'Move focus to the time input.' },
            { key: 'Shift + Tab', action: 'Move focus away from the time input to the previous element.' },
            { key: 'Type digits', action: 'Enter hour, minute, or second values directly into the focused segment.' },
            { key: 'Arrow Up / Down', action: 'Increment or decrement the currently focused time segment (hours, minutes, seconds, or AM/PM).' },
        ...
        <AriaTable
          rows={[
            ...
            },
            {
              ...
              note: 'Associates the visible label with the inner <input type="time"> via Shadow DOM id association.',
            },
          ]}
        />
      </Section>
      ...
      <Section title="Label association">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The label is associated with the inner <Code>{'<input type="time">'}</Code> via Shadow DOM
          id association. Screen readers announce the label and the current time value on focus.
        </p>
      </Section>

      <Section title="Native time picker">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The component delegates to the browser's native time picker. Users can navigate between hour,
          minute, and second segments with arrow keys or by typing. AM/PM segment is navigable when the
          locale uses 12-hour format.
        </p>
      </Section>
      ...
      <Section title="step and granularity">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The <Code>step</Code> value (in seconds) affects the minute/second segments shown. When
          <Code>step</Code> is set to a value that includes seconds (e.g. <Code>30</Code>), the seconds
          segment becomes visible in the picker. Screen readers announce the step via the spinbutton role
          of each segment.
        </p>
      </Section>
      ...
      <Section title="Focus management">
        <p className="text-sm text-[var(--diwa-text-secondary)]">
          <Code>{'delegatesFocus: true'}</Code> routes host focus to the time input. Reduced-motion
      ...
      <Section title="Best practices">
        <ul className="space-y-4 text-sm text-[var(--diwa-text-secondary)]">
          ...
          <li>
            <strong className="text-[var(--diwa-text-primary)]">Step and granularity</strong>{' '}
            — the{' '}
            <code className="px-1 py-0.5 rounded bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] text-xs font-mono">step</code>{' '}
            attribute (in seconds) controls which time segments are visible. When seconds precision is needed, set step to a value that exposes the seconds segment; omitting it shows hours and minutes only.
          </li>
          <li>
