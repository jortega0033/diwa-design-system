...
export default function InputPasswordAccessibilityPage() {
  return (
    <div className="max-w-3xl">
      <Section title="Keyboard interaction">
        <KeyboardTable
          rows={[
            { key: 'Tab', action: 'Move focus to the input or the show/hide toggle button (if visible).' },
            { key: 'Shift + Tab', action: 'Move focus to the previous focusable element.' },
            { key: 'Type', action: 'Enter text directly into the password field.' },
      ...
      <Section title="Screen reader behaviour">
        ...
        <AriaTable
          rows={[
            {
              property: 'aria-label (toggle button)',
              value: 'Switches between "Show password" / "Hide password"',
              note: 'Dynamically updated so screen readers announce the current toggle state.',
            },
            {
            ...
            },
            {
              ...
              note: 'Associates the visible label with the inner input regardless of its current type (password or text).',
            },
          ]}
        />
      </Section>
      ...
      <Section title="Label association">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The label is associated with the inner <Code>{'<input type="password">'}</Code> or{' '}
          <Code>{'<input type="text">'}</Code> (when revealed) via Shadow DOM id association. Screen
          readers announce the label on focus in both states.
        </p>
      </Section>

      <Section title="Visibility toggle button">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          When <Code>showToggle</Code> is <Code>true</Code>, an icon button is rendered inside the field.
          Its <Code>aria-label</Code> dynamically changes between <Code>"Show password"</Code> and{' '}
          <Code>"Hide password"</Code> to reflect current state. It is keyboard focusable with a visible
          focus ring applied by <Code>getFocusStyle</Code>.
        </p>
        <p className="text-sm text-[var(--diwa-text-secondary)]">
          Toggling switches the inner input <Code>type</Code> between <Code>password</Code> and{' '}
          <Code>text</Code>. Screen readers re-announce the field contents when the type switches to
          text — inform users via the accessible description if this is a concern.
        </p>
      </Section>
      ...
      <Section title="Validation state">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          <Code>state="error"</Code> sets <Code>aria-invalid="true"</Code> and links the error{' '}
      ...
      <Section title="Best practices">
        <ul className="space-y-4 text-sm text-[var(--diwa-text-secondary)]">
          ...
          <li>
            <strong className="text-[var(--diwa-text-primary)]">Visibility re-announcement</strong>{' '}
            — toggling the input type from{' '}
            <code className="px-1 py-0.5 rounded bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] text-xs font-mono">password</code>{' '}
            to{' '}
            <code className="px-1 py-0.5 rounded bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] text-xs font-mono">text</code>{' '}
            causes screen readers to re-announce the field value. Inform users via a description if revealing the password is a concern in your context.
          </li>
          <li>
