...
export default function InputTextAccessibilityPage() {
  return (
    <div className="max-w-3xl">
      <Section title="Keyboard interaction">
        <KeyboardTable
          rows={[
            ...
            { key: 'Type', action: 'Enter text directly into the field.' },
        ...
        <AriaTable
          rows={[
            ...
            },
            {
              ...
              note: 'Links the error (when active) or the description text. Description is hidden when a message is shown to avoid redundant announcements.',
            },
            {
              property: 'aria-label / aria-labelledby',
              value: 'From label prop',
              note: 'Associates the visible label with the inner <input>. Still present in the accessibility tree when hideLabel is true.',
            },
          ]}
        />
      </Section>
      ...
      <Section title="Label association">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The <Code>label</Code> prop renders a visible <Code>{'<label>'}</Code> inside Shadow DOM,
          associated with the inner <Code>{'<input>'}</Code> via matching <Code>id</Code> / <Code>htmlFor</Code>.
          Screen readers announce the label on focus.
        </p>
        <p className="text-sm text-[var(--diwa-text-secondary)]">
          When <Code>hideLabel</Code> is <Code>true</Code>, the label is visually hidden but still
          present in the accessibility tree and used as the accessible name.
        </p>
      </Section>

      <Section title="Required fields">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          Setting <Code>required</Code> adds a visual asterisk (<Code>*</Code>) next to the label and
          sets <Code>aria-required="true"</Code> on the inner <Code>{'<input>'}</Code>.
        </p>
      </Section>

      <Section title="Validation state">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          When <Code>state="error"</Code>, the component sets <Code>aria-invalid="true"</Code> on the
          input. The <Code>message</Code> element is associated via <Code>aria-describedby</Code>,
          so screen readers announce the error text after the field label.
        </p>
      </Section>

      <Section title="Description">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          A non-empty <Code>description</Code> is also linked via <Code>aria-describedby</Code>. When
          a <Code>message</Code> is present the description is hidden to avoid redundant announcements.
        </p>
      </Section>

      <Section title="Focus management">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The component uses <Code>{'delegatesFocus: true'}</Code> in its Shadow DOM, so calling{' '}
          <Code>.focus()</Code> on the host element or tabbing to it forwards focus directly to the
          inner <Code>{'<input>'}</Code>.
        </p>
      </Section>

      <Section title="Reduced motion">
        <p className="text-sm text-[var(--diwa-text-secondary)]">
          All transitions respect the <Code>prefers-reduced-motion: reduce</Code> media query.
        </p>
      </Section>
      ...
      <Section title="Best practices">
        <ul className="space-y-4 text-sm text-[var(--diwa-text-secondary)]">
          ...
          <li>
            <strong className="text-[var(--diwa-text-primary)]">Description vs. message</strong>{' '}
            — avoid providing the same text in both{' '}
            ...
            — when a message is present, the description is hidden to prevent redundant screen reader announcements.
          </li>
          <li>
