...
export default function InputWeekAccessibilityPage() {
  return (
    <div className="max-w-3xl">
      <Section title="Keyboard interaction">
        <KeyboardTable
          rows={[
            { key: 'Tab', action: 'Move focus to the week input.' },
            { key: 'Shift + Tab', action: 'Move focus away from the week input to the previous element.' },
            { key: 'Type digits', action: 'Enter week and year values directly into the focused segment.' },
            { key: 'Arrow Up / Down', action: 'Increment or decrement the currently focused week or year segment.' },
            { key: 'Enter', action: 'Open or close the native browser week picker dialog (Chromium only).' },
          ]}
        />
      </Section>
        ...
        <AriaTable
          rows={[
            ...
            },
            {
              ...
              note: 'Associates the visible label with the inner <input type="week"> via Shadow DOM id association.',
            },
          ]}
        />
      </Section>
      ...
      <Section title="Label association">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The label is associated with the inner <Code>{'<input type="week">'}</Code> via Shadow DOM
          id association. Screen readers announce the label and the selected week on focus.
        </p>
      </Section>

      <Section title="Native week picker">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          Chrome and Edge provide a native week picker. Firefox and Safari fall back to a plain text
          input. For non-Chromium browsers, communicate the expected format (<Code>YYYY-Www</Code>, e.g.{' '}
          <Code>2025-W24</Code>) via the <Code>description</Code> prop so users know what to type.
        </p>
      </Section>
      ...
      <Section title="Best practices">
        <ul className="space-y-4 text-sm text-[var(--diwa-text-secondary)]">
          ...
          <li>
            <strong className="text-[var(--diwa-text-primary)]">Browser support</strong>{' '}
            — Chrome and Edge provide a native week picker; Firefox and Safari fall back to a plain text input. Always provide a{' '}
            ...
            communicating the expected format ({' '}
            <code className="px-1 py-0.5 rounded bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] text-xs font-mono">YYYY-Www</code>, e.g.{' '}
            <code className="px-1 py-0.5 rounded bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] text-xs font-mono">2025-W24</code>) for non-Chromium users.
          </li>
          <li>
