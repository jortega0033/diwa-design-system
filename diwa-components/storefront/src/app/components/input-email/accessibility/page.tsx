...
export default function InputEmailAccessibilityPage() {
  return (
    <div className="max-w-3xl">
      <Section title="Keyboard interaction">
        <KeyboardTable
          rows={[
            { key: 'Tab', action: 'Move focus to the email input.' },
            { key: 'Shift + Tab', action: 'Move focus away from the email input to the previous element.' },
            { key: 'Type', action: 'Enter an email address. The browser validates format on submit.' },
          ]}
        />
      </Section>
        ...
        <AriaTable
          rows={[
            ...
            },
            {
              ...
              note: 'Associates the visible label with the inner <input type="email">. Set hideLabel to visually hide it while keeping it accessible.',
            },
          ]}
        />
      </Section>
      ...
      <Section title="Label association">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          ...
          associated with the inner <Code>{'<input type="email">'}</Code>. Set <Code>hideLabel</Code> to
          visually hide it while keeping it accessible.
        </p>
      </Section>

      <Section title="Input type semantics">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          Using <Code>type="email"</Code> gives mobile browsers the email keyboard layout
          (with <Code>@</Code> and <Code>.com</Code> shortcuts) and triggers native format validation
          before submit. Screen readers may announce additional context about the expected format.
        </p>
      </Section>

      <Section title="Required fields">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          Setting <Code>required</Code> adds a visual asterisk and sets <Code>aria-required="true"</Code> on
          the inner input.
        </p>
      </Section>
      ...
      <Section title="Validation state">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          When <Code>state="error"</Code>, <Code>aria-invalid="true"</Code> is set on the input. The{' '}
          <Code>message</Code> element is linked via <Code>aria-describedby</Code> so the error is
      ...
      <Section title="Focus management">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          ...
          styles are applied via <Code>prefers-reduced-motion</Code>.
        </p>
      </Section>
      ...
      <Section title="Best practices">
        <ul className="space-y-4 text-sm text-[var(--diwa-text-secondary)]">
          ...
          <li>
            <strong className="text-[var(--diwa-text-primary)]">Mobile keyboards</strong>{' '}
            —{' '}
            <code className="px-1 py-0.5 rounded bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] text-xs font-mono">type="email"</code>{' '}
            activates an email-optimised keyboard (with @ and . shortcuts) on mobile devices automatically, reducing input errors.
          </li>
          <li>
