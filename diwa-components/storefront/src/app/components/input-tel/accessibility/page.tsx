...
export default function InputTelAccessibilityPage() {
  return (
    <div className="max-w-3xl">
      <Section title="Keyboard interaction">
        <KeyboardTable
          rows={[
            ...
            { key: 'Type', action: 'Enter a telephone number directly into the field.' },
        ...
        <AriaTable
          rows={[
            ...
            },
            {
              ...
              note: 'Associates the visible label with the inner <input type="tel"> via Shadow DOM id association.',
            },
          ]}
        />
      </Section>
      ...
      <Section title="Label association">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The label is associated with the inner <Code>{'<input type="tel">'}</Code> via Shadow DOM
          label/id pairing. Use <Code>hideLabel</Code> when visual context makes the label redundant
          while keeping it for screen readers.
        </p>
      </Section>

      <Section title="Input type semantics">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          <Code>type="tel"</Code> opens the telephone numeric keyboard on mobile devices. Unlike{' '}
          <Code>type="number"</Code>, it accepts leading zeros and formatting characters (spaces,
          hyphens, parentheses), making it appropriate for phone numbers.
        </p>
      </Section>

      <Section title="Required fields">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          <Code>required</Code> adds a visible asterisk and <Code>aria-required="true"</Code> on the
          inner input.
        </p>
      </Section>
      ...
      <Section title="Focus management">
        <p className="text-sm text-[var(--diwa-text-secondary)]">
          <Code>{'delegatesFocus: true'}</Code> ensures host-level focus delegates to the inner input.
          Reduced-motion styles are applied via <Code>prefers-reduced-motion</Code>.
        </p>
      </Section>
      ...
      <Section title="Best practices">
        <ul className="space-y-4 text-sm text-[var(--diwa-text-secondary)]">
          ...
          <li>
            ...
            —{' '}
            <code className="px-1 py-0.5 rounded bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] text-xs font-mono">type="tel"</code>{' '}
            activates a telephone-optimised numeric keypad on mobile devices, reducing input errors for phone number fields.
          </li>
          <li>
