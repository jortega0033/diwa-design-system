...
export default function InputMonthAccessibilityPage() {
  return (
    <div className="max-w-3xl">
      <Section title="Keyboard interaction">
        <KeyboardTable
          rows={[
            { key: 'Tab', action: 'Move focus to the month input.' },
            { key: 'Shift + Tab', action: 'Move focus away from the month input to the previous element.' },
            { key: 'Type digits', action: 'Enter month and year values directly by typing numeric values into the focused segment.' },
            { key: 'Arrow Up / Down', action: 'Increment or decrement the currently focused month or year segment.' },
            { key: 'Enter', action: 'Open or close the native browser month picker dialog.' },
          ]}
        />
      </Section>
        ...
        <AriaTable
          rows={[
            ...
            },
            {
              ...
              note: 'Associates the visible label with the inner <input type="month"> via Shadow DOM id association.',
            },
          ]}
        />
      </Section>
      ...
      <Section title="Label association">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The label is associated with the inner <Code>{'<input type="month">'}</Code> via Shadow DOM
          id association. Screen readers announce the label and the selected month on focus.
        </p>
      </Section>

      <Section title="Native month picker">
        <p className="text-sm text-[var(--diwa-text-secondary)] mb-3">
          The component delegates to the browser's native month picker (Chrome / Edge). Users can
          navigate month and year segments with arrow keys or by typing. Safari renders a plain text
          input as a fallback — communicate the <Code>YYYY-MM</Code> format via the{' '}
          <Code>description</Code> prop for those users.
        </p>
      </Section>
      ...
      <Section title="Best practices">
        <ul className="space-y-4 text-sm text-[var(--diwa-text-secondary)]">
          ...
          <li>
            ...
            — Safari renders a plain text input instead of a native month picker. Always provide a{' '}
            ...
            <code className="px-1 py-0.5 rounded bg-[var(--diwa-bg-surface)] border border-[var(--diwa-border)] text-xs font-mono">YYYY-MM</code>) so users know what to type.
          </li>
          <li>
